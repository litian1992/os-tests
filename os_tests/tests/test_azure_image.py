...
hypervkvpd,hyperv-daemons-license,hypervfcopyd,hypervvssd,hyperv-daemons'''
    ...
    def test_check_rhui_cert(self):
        ...
        # else:
        #     cmd = 'sudo rpm -q redhat-release'
        cmd = "timestamp=$(sudo rct cat-cert /etc/pki/rhui/product/*.crt|grep 'End Date'|awk '{print $3}');date -d $timestamp +%s"
    ...
    def test_check_dracut_conf(self):
        ...
        cmd = "ls /etc/dracut.conf.d/ | grep -v nvme.conf"
    ...
    def test_check_hostkey_permission(self):
        ...
        cmd = "ls -l /etc/ssh/{ssh_host_ecdsa_key,ssh_host_ed25519_key,ssh_host_rsa_key}|awk '{print $1$3$4}'|uniq|sed 's/\.//g'"
...
'''
    ...
    def test_check_cloud_cfg(self):
        ...
        if self.rhel_x_version >= 9:
            ...
            self.skipTest('Only support in RHEL-9+')
...
'''
    ...
    def test_check_disk_azure_is_directory(self):
        '''
        BZ 2165042
        /dev/disk/azure should be a directory, not link.
        '''
        cmd = "[[ -d /dev/disk/azure ]] || false"
        utils_lib.run_cmd(self, cmd, expect_ret=0, msg='Check /dev/disk/azure is a directory.')

    def test_check_no_ssh_key_left(self):
        '''
        BZ 2127969
